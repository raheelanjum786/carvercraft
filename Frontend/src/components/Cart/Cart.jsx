...
const Cart = () => {
    ...
    const fetchCartItems = async () => {
      ...
      try {
        const response = await api.get(
          `http://51.21.182.124:4000/api/cart/get/${user.id}`
        );
  ...
  const handleQuantityChange = async (cartItemId, quantity) => {
    try {
      await api.put("http://51.21.182.124:4000/api/cart/update", {
        cartItemId,
        quantity,
      });
  ...
  const handleRemoveItem = async (cartItemId) => {
    try {
      await api.delete(
        `http://51.21.182.124:4000/api/cart/remove/${cartItemId}`
      );
      setCartItems((prevItems) =>
  ...
  const handleProceedToCheckout = async () => {
    ...
    try {
      ...
      await api.delete(`http://51.21.182.124:4000/api/cart/clear/${user.id}`);
      setCartItems([]);
