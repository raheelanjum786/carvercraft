...
const AdminCardTypes = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await api.put(
          `http://51.21.182.124/api/api/cardTypes/update/${editingId}`,
          submitData,
          {
        ...
        await api.post(
          "http://51.21.182.124/api/api/cardTypes/create",
          submitData,
          {
            headers: { "Content-Type": "multipart/form-data" },
          }
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this card type?")) {
      try {
        await api.delete(`http://51.21.182.124/api/api/cardTypes/${id}`);
