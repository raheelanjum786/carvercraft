...
const AdminProductList = () => {
    ...
    const fetchData = async () => {
      try {
        const [productsRes, categoriesRes] = await Promise.all([
          api.get("http://51.21.182.124/api/api/products/get"),
          api.get("http://51.21.182.124/api/api/categories/get"),
        ]);
  ...
  const handleDelete = async (productId) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      try {
        await api.delete(`http://51.21.182.124/api/api/products/${productId}`);
  ...
  const handleUpdate = async (updatedProduct) => {
    try {
      ...
      const response = await api.put(
        `http://51.21.182.124/api/api/products/update/${updatedProduct.id}`,
        productData,
        {
          headers: {
                            ...
                            try {
                              ...
                              return images.map((imageUrl, index) => (
                                <div key={index} className="relative">
                                  <img
                                    src={`http://51.21.182.124/api/api${imageUrl}`}
                                    alt={`Product ${index + 1}`}
