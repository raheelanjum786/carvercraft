...
const AdminProductList = () => {
    ...
    const fetchData = async () => {
      try {
        const [productsRes, categoriesRes] = await Promise.all([
          api.get("http://13.60.44.89:4000/api/products/get"),
          api.get("http://13.60.44.89:4000/api/categories/get"),
        ]);
  ...
  const handleDelete = async (productId) => {
    if (window.confirm("Are you sure you want to delete this product?")) {
      try {
        await api.delete(`http://13.60.44.89:4000/api/products/${productId}`);
  ...
  const handleUpdate = async (updatedProduct) => {
    try {
      ...
      const response = await api.put(
        `http://13.60.44.89:4000/api/products/update/${updatedProduct.id}`,
        productData,
        {
          headers: {
                            ...
                            try {
                              ...
                              return images.map((imageUrl, index) => (
                                <div key={index} className="relative">
                                  <img
                                    src={`http://13.60.44.89:4000/api${imageUrl}`}
                                    alt={`Product ${index + 1}`}
