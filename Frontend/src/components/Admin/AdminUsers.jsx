...
const AdminUsers = () => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await api.get(
          "http://13.60.44.89:4000/api/auth/users/get-users"
        );
  ...
  const handleDelete = async (userId, e) => {
    e.stopPropagation(); // Stop event propagation
    try {
      await api.delete(`http://13.60.44.89:4000/api/auth/users/${userId}`);
  ...
  const handleModalOk = async () => {
    try {
      await api.put(
        `http://13.60.44.89:4000/api/auth/users/${editingUser._id}`,
        editingUser
      );
