...
const AdminUsers = () => {
    ...
    const fetchUsers = async () => {
      try {
        const response = await api.get(
          "http://51.21.182.124/api/api/auth/users/get-users"
        );
  ...
  const handleDelete = async (userId, e) => {
    e.stopPropagation(); // Stop event propagation
    try {
      await api.delete(`http://51.21.182.124/api/api/auth/users/${userId}`);
  ...
  const handleModalOk = async () => {
    try {
      await api.put(
        `http://51.21.182.124/api/api/auth/users/${editingUser._id}`,
        editingUser
      );
